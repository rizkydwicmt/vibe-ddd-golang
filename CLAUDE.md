...
Authentication and field-level encryption are intentionally **not** included here — this is the
plaintext starting template. Opaque cursor protection remains part of generic pagination.
Two example domains (`user`, `payment`) demonstrate the full pattern.
...
internal/pkg/reqbind       | body/query binding seam                            | Bind, BindQuery, Query
